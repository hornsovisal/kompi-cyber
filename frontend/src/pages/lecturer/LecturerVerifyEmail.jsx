...
export default function LecturerVerifyEmail() {
  ...
  useEffect(() => {
    ...
    const verifyEmail = async () => {
      try {
        const response = await axios.post('/api/lecturer/verify-email', { token });
        setStatus('success');
        setMessage('Email verified successfully! You can now log in.');
        setTimeout(() => navigate('/lecturer/login'), 3000);
      } catch (error) {
        setStatus('error');
            ...
            {status === 'error' && (
              <div>
                ...
                <button
                  ...
                  className="w-full rounded-2xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-200"
                >
                  Go to Login
                </button>
              </div>
